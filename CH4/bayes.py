...
def calcMostFreq(vocabList,fullText):
    ...
    for token in vocabList:
        freqDict[token] = fullText.count(token) #Record each word appeared times in the vocabulary  
    sortedFreq = sorted(freqDict.items(),key=operator.itemgetter(1),reverse=True) # ranking from top to bottom    
    return sortedFreq[:30] # Get the highest 30 
...
def localWords(feed1,feed0):
    ...
    for pairW in top30Words:
        if pairW[0] in vocabList:
            vocabList.remove(pairW[0]) # pairW[0] get the token of dict
    trainingSet = [ i for i in range(2*minLen)]
    testSet = []
    for i in range(20):
        randIndex = int(np.random.uniform(0,len(trainingSet)))
        testSet.append(trainingSet[randIndex])    # to randomly get the test data set
        del(trainingSet[randIndex])
    trainMat = []
    trainClasses = []
